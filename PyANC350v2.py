...
import PyANC350v2.ANC350v2lib as ANC
import ctypes, math
...
class Positioner:
	...
	def connect(self):
		...
		try:
			ANC.positionerConnect(0,ctypes.byref(self.handle)) #0 means "first device"
	...
	def externalStepFwdInput(self, axis, input_trigger):
		...
		'''
		ANC.positionerExternalStepFwdInput(self.handle, axis, nput_trigger)
	...
	def moveContinuous(self, axis, direction):
		...
		'''
		ANC.positionerMoveContinuous(self.handle, axis, direction)
