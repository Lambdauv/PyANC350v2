...
import ANC350v2lib, ctypes, math
...
class Positioner:
	...
	def acInEnable(self, axis, state):
		...
		'''
		ANC350v2lib.positionerAcInEnable(self.handle,axis,ctypes.c_bool(state))

	def amplitude(self, axis, amp):
		'''
		set the amplitude setpoint in mV
		'''
		ANC350v2lib.positionerAmplitude(self.handle, axis, amp)
	...
	def amplitudeControl(self, axis, mode):
		...
		'''
		ANC350v2lib.positionerAmplitudeControl(self.handle, axis, mode)
	...
	def bandwidthLimitEnable(self, axis, state):
		...
		'''
		ANC350v2lib.positionerBandwidthLimitEnable(self.handle,axis,ctypes.c_bool(state))

	def capMeasure(self, axis):
		'''
		determines the capacitance of the piezo addressed by axis
		'''
		self.status = ANC350v2lib.Int32(0)
		ANC350v2lib.positionerCapMeasure(self.handle, axis, ctypes.byref(self.status))
	...
	def check(self):
		...
		'''
		self.posinf = ANC350v2lib.PositionerInfo() #create PositionerInfo Struct
		self.numconnected = ANC350v2lib.positionerCheck(ctypes.byref(self.posinf)) #look for positioners!
	...
	def clearStopDetection(self, axis):
		...
		'''
		ANC350v2lib.positionerClearStopDetection(self.handle,axis)

	def close(self):
		'''
		closes connection to ANC350 device
		'''
		ANC350v2lib.positionerClose(self.handle)

	def connect(self):
		'''
		Establishes connection to first device found
		'''
		self.handle = ANC350v2lib.Int32(0)
		try:
			ANC350v2lib.positionerConnect(0,ctypes.byref(self.handle)) #0 means "first device"
	...
	def dcInEnable(self, axis, state):
		...
		'''
		ANC350v2lib.positionerDcInEnable(self.handle, axis, ctypes.c_bool(state))

	def dcLevel(self, axis, dclev):
		'''
		sets the dc level of selected axis. dclevel in mV
		'''
		ANC350v2lib.positionerDCLevel(self.handle, axis, dclev)

	def dutyCycleEnable(self, state):
		'''
		controls duty cycle mode
		'''
		ANC350v2lib.positionerDutyCycleEnable(self.handle, ctypes.c_bool(state))

	def dutyCycleOffTime(self, value):
		'''
		sets duty cycle off time
		'''
		ANC350v2lib.positionerDutyCycleOffTime(self.handle, value)

	def dutyCyclePeriod(self, value):
		'''
		sets duty cycle period
		'''
		ANC350v2lib.positionerDutyCyclePeriod(self.handle, value)
	...
	def externalStepBkwInput(self, axis, input_trigger):
		...
		'''
		ANC350v2lib.positionerExternalStepBkwInput(self.handle, axis, input_trigger)
	...
	def externalStepFwdInput(self, axis, input_trigger):
		...
		'''
		ANC350v2lib.positionerExternalStepFwdInput(self.handle, axis, nput_trigger)
	...
	def externalStepInputEdge(self, axis, edge):
		...
		'''
		ANC350v2lib.positionerExternalStepInputEdge(self.handle, axis, edge)

	def frequency(self, axis, freq):
		'''
		sets the frequency of selected axis. frequency in Hz
		'''
		ANC350v2lib.positionerFrequency(self.handle, axis, freq)
	...
	def getAcInEnable(self, axis):
		...
		ANC350v2lib.positionerGetAcInEnable(self.handle,axis,ctypes.byref(self.status))
		return self.status.value
	...
	def getAmplitude(self, axis):
		...
		'''
		self.status = ANC350v2lib.Int32(0)
		ANC350v2lib.positionerGetAmplitude(self.handle, axis, ctypes.byref(self.status))
		return self.status.value
	...
	def getBandwidthLimitEnable(self, axis):
		...
		'''
		self.status = ctypes.c_bool(None)
		ANC350v2lib.positionerGetBandwidthLimitEnable(self.handle, axis, ctypes.byref(self.status))
		return self.status.value
	...
	def getDcInEnable(self, axis):
		...
		'''
		self.status = ctypes.c_bool(None)
		ANC350v2lib.positionerGetDcInEnable(self.handle,axis,ctypes.byref(self.status))
		return self.status.value
	...
	def getDcLevel(self, axis):
		...
		'''
		self.dclev = ANC350v2lib.Int32(0)
		ANC350v2lib.positionerGetDcLevel(self.handle, axis, ctypes.byref(self.dclev))
	...
	def getFrequency(self, axis):
		...
		'''
		self.freq = ANC350v2lib.Int32(0)
		ANC350v2lib.positionerGetFrequency(self.handle, axis, ctypes.byref(self.freq))
	...
	def getIntEnable(self, axis):
		...
		'''
		self.status = ctypes.c_bool(None)
		ANC350v2lib.positionerGetIntEnable(self.handle, axis, ctypes.byref(self.status))
		return self.status.value
	...
	def getPosition(self, axis):
		...
		'''
		self.pos = ANC350v2lib.Int32(0)
		ANC350v2lib.positionerGetPosition(self.handle, axis, ctypes.byref(self.pos))
	...
	def getReference(self, axis):
		...
		ANC350v2lib.positionerGetReference(self.handle, axis, ctypes.byref(self.pos), ctypes.byref(self.validity))
	...
	def getReferenceRotCount(self, axis):
		...
		determines actual position of addressed axis
		'''
		self.rotcount = ANC350v2lib.Int32(0)
		ANC350v2lib.positionerGetReferenceRotCount(self.handle, axis, ctypes.byref(self.rotcount))
	...
	def getRotCount(self, axis):
		...
		'''
		self.rotcount = ANC350v2lib.Int32(0)
		ANC350v2lib.positionerGetRotCount(self.handle, axis, ctypes.byref(self.rotcount))
		return self.rotcount.value
	...
	def getSpeed(self, axis):
		...
		'''
		self.spd = ANC350v2lib.Int32(0)
		ANC350v2lib.positionerGetSpeed(self.handle, axis, ctypes.byref(self.spd))
	...
	def getStatus(self, axis):
		...
		'''
		self.status = ANC350v2lib.Int32(0)
		ANC350v2lib.positionerGetStatus(self.handle, axis, ctypes.byref(self.status))
		return self.status.value
	...
	def getStepwidth(self, axis):
		...
		'''
		self.stepwdth = ANC350v2lib.Int32(0)
		ANC350v2lib.positionerGetStepwidth(self.handle, axis, ctypes.byref(self.stepwdth))
	...
	def intEnable(self, axis, state):
		...
		'''
		ANC350v2lib.positionerIntEnable(self.handle, axis, ctypes.c_bool(state))
	...
	def load(self, axis, filename):
		...
		'''
		ANC350v2lib.positionerLoad(self.handle, axis, ctypes.byref(ctypes.char(filename)))
	...
	def moveAbsolute(self, axis, position, rotcount=0):
		...
		'''
		ANC350v2lib.positionerMoveAbsolute(self.handle, axis, position, rotcount)
	...
	def moveAbsoluteSync(self, bitmask_of_axes):
		...
		'''
		ANC350v2lib.positionerMoveAbsoluteSync(self.handle, bitmask_of_axes)
	...
	def moveContinuous(self, axis, direction):
		...
		'''
		ANC350v2lib.positionerMoveContinuous(self.handle, axis, direction)

	def moveReference(self, axis):
		'''
		starts approach to reference position. previous movement will be stopped.
		'''
		ANC350v2lib.positionerMoveReference(self.handle, axis)
	...
	def moveRelative(self, axis, position, rotcount=0):
		...
		'''
		ANC350v2lib.positionerMoveRelative(self.handle, axis, position, rotcount)
	...
	def moveSingleStep(self, axis, direction):
		...
		'''
		ANC350v2lib.positionerMoveSingleStep(self.handle, axis, direction)
	...
	def quadratureAxis(self, quadratureno, axis):
		...
		'''
		ANC350v2lib.positionerQuadratureAxis(self.handle, quadratureno, axis)
	...
	def quadratureInputPeriod(self, quadratureno, period):
		...
		'''
		ANC350v2lib.positionerQuadratureInputPeriod(self.handle, quadratureno, period)
	...
	def quadratureOutputPeriod(self, quadratureno, period):
		...
		'''
		ANC350v2lib.positionerQuadratureOutputPeriod(self.handle, quadratureno, period)

	def resetPosition(self, axis):
		'''
		sets the origin to the actual position
		'''
		ANC350v2lib.positionerResetPosition(self.handle, axis)
	...
	def sensorPowerGroupA(self, state):
		...
		'''
		ANC350v2lib.positionerSensorPowerGroupA(self.handle, ctypes.c_bool(state))
	...
	def sensorPowerGroupB(self, state):
		...
		'''
		ANC350v2lib.positionerSensorPowerGroupB(self.handle, ctypes.c_bool(state))

	def setHardwareId(self, hwid):
		'''
		sets the hardware ID for the device (used to differentiate multiple devices)
		'''
		ANC350v2lib.positionerSetHardwareId(self.handle, hwid)

	def setOutput(self, axis, state):
		'''
		activates/deactivates the addressed axis
		'''
		ANC350v2lib.positionerSetOutput(self.handle, axis, ctypes.c_bool(state))
	...
	def setStopDetectionSticky(self, axis, state):
		...
		'''
		ANC350v2lib.positionerSetStopDetectionSticky(self.handle, axis, state)

	def setTargetGround(self, axis, state):
		'''
		when enabled, actor voltage set to zero after closed-loop positioning finished
		'''
		ANC350v2lib.positionerSetTargetGround(self.handle, axis, ctypes.c_bool(state))

	def setTargetPos(self, axis, pos, rotcount=0):
		'''
		sets target position for use with .moveAbsoluteSync()
		'''
		ANC350v2lib.positionerSetTargetPos(self.handle, axis, pos, rotcount)
	...
	def singleCircleMode(self, axis, state):
		...
		'''
		ANC350v2lib.positionerSingleCircleMode(self.handle, axis, ctypes.c_bool(state))

	def staticAmplitude(self, amp):
		'''
		sets output voltage for resistive sensors
		'''
		ANC350v2lib.positionerStaticAmplitude(self.handle, amp)
	...
	def stepCount(self, axis, stps):
		...
		'''
		ANC350v2lib.positionerStepCount(self.handle, axis, stps)
	...
	def stopApproach(self, axis):
		...
		'''
		ANC350v2lib.positionerStopApproach(self.handle, axis)

	def stopDetection(self, axis, state):
		'''
		switches stop detection on/off
		'''
		ANC350v2lib.positionerStopDetection(self.handle, axis, ctypes.c_bool(state))

	def stopMoving(self, axis):
		'''
		stops any positioning, DC level of affected axis is set to zero after stopping.
		'''
		ANC350v2lib.positionerStopMoving(self.handle, axis)
	...
	def triggerModeIn(self, mode):
		...
		'''
		ANC350v2lib.positionerTriggerModeIn(self.handle, mode)
	...
	def triggerModeOut(self, mode):
		...
		'''
		ANC350v2lib.positionerTriggerModeOut(self.handle, mode)
	...
	def triggerPolarity(self, triggerno, polarity):
		...
		'''
		ANC350v2lib.positionerTriggerPolarity(self.handle,triggerno,polarity)
	...
	def updateAbsolute(self, axis, position):
		...
		'''
		ANC350v2lib.positionerUpdateAbsolute(self.handle,axis,position)
